...
// hostTextOrderKeys names the entries in hostAllowedOrderKeys whose underlying
// columns are text/varchar. Cursor pagination must bind cursor values as
// strings for these — pgx rejects an int8-bind against a text column even when
// the cursor value parses as a number (see appendListOptionsWithCursorToSQLSecure).
var hostTextOrderKeys = []string{
	"hostname", "uuid", "computer_name", "platform", "os_version", "osquery_version",
	"cpu_type", "hardware_vendor", "hardware_model", "hardware_serial", "team_name",
	"primary_ip", "primary_mac", "public_ip", "orbit_version", "fleet_desktop_version",
	"display_name",
}
...
var batchScriptHostTextOrderKeys = []string{"display_name", "hostname"}
...
func saveHostPackStatsDB(ctx context.Context, db *sqlx.DB, dialect DialectHelper, teamID *uint, hostID uint, stats []fleet.PackStats) error {
		...
		if dialect.IsPostgres() {
			...
			for i := 0; i < scheduledQueriesQueryCount; i++ {
				...
				selectParts = append(selectParts,
					"SELECT q.id, ?::bigint,?::bigint,?::boolean,?::bigint,?::bigint,?::timestamptz,?::bigint,?::bigint,?::bigint,?::bigint FROM queries q WHERE COALESCE(q.team_id, 0) = ?::bigint AND q.name = ?::text")
			...
			sql := `INSERT INTO scheduled_query_stats (
					scheduled_query_id,
					host_id,
					average_memory,
					denylisted,
				...
				` + selectSQL + ` ` + dialect.OnDuplicateKey("host_id,scheduled_query_id,query_type", `
					scheduled_query_id = EXCLUDED.scheduled_query_id,
					host_id = EXCLUDED.host_id,
					average_memory = EXCLUDED.average_memory,
	...
	if userPacksQueryCount > 0 {
		...
		if dialect.IsPostgres() {
			// Use INSERT...SELECT form so that rows where the subquery returns
			// no match are naturally excluded (avoids NOT NULL violations on PG).
			// Wrap in a subquery with DISTINCT ON to prevent "cannot affect row
			...
			for i := 0; i < userPacksQueryCount; i++ {
				base := i * argsPerRow
				selectParts = append(selectParts,
					"SELECT sq.query_id, ?::bigint,?::bigint,?::boolean,?::bigint,?::bigint,?::timestamptz,?::bigint,?::bigint,?::bigint,?::bigint FROM scheduled_queries sq JOIN packs p ON (sq.pack_id = p.id) WHERE p.pack_type IS NULL AND p.name = ?::text AND sq.name = ?::text")
				// Reorder: value args first (host_id..wall_time), then subquery args (packName, sqName)
				reorderedArgs = append(reorderedArgs, userPacksArgs[base+2:base+argsPerRow]...)
				reorderedArgs = append(reorderedArgs, userPacksArgs[base:base+2]...)
			}
			innerSQL := strings.Join(selectParts, " UNION ALL ")
			sql := `INSERT INTO scheduled_query_stats (
					scheduled_query_id,
					host_id,
					average_memory,
				...
				SELECT DISTINCT ON (scheduled_query_id)
					scheduled_query_id::bigint, host_id::bigint, average_memory::bigint, denylisted::boolean,
					executions::bigint, schedule_interval::bigint, last_executed::timestamptz, output_size::bigint,
					system_time::bigint, user_time::bigint, wall_time::bigint
				FROM (` + innerSQL + `) AS src(scheduled_query_id, host_id, average_memory, denylisted, executions, schedule_interval, last_executed, output_size, system_time, user_time, wall_time)
				` + dialect.OnDuplicateKey("host_id,scheduled_query_id,query_type", `
					scheduled_query_id = EXCLUDED.scheduled_query_id,
					host_id = EXCLUDED.host_id,
					average_memory = EXCLUDED.average_memory,
					denylisted = EXCLUDED.denylisted,
...
// hostMDMSelectSQL returns the SQL fragment used to construct the JSON object
// of MDM host data. It assumes that hostMDMJoin is included in the query.
func hostMDMSelectSQL(dialect DialectHelper) string {
	return `,
	` + dialect.JSONObjectFunc() + `(
...
func (ds *Datastore) ListHosts(ctx context.Context, filter fleet.TeamFilter, opt fleet.HostListOptions) ([]*fleet.Host, error) {
	...
	sql += hostMDMSelectSQL(ds.dialect)
	...
	if len(opt.AdditionalFilters) == 1 && opt.AdditionalFilters[0] == "*" {
		...
		sql += `, (SELECT ` + ds.dialect.JSONObjectFunc() + `(
			`
...
WHERE
	...
	sqlStmt, queryParams, listOptsErr = appendListOptionsWithCursorToSQLSecure(sqlStmt, queryParams, &opt, batchScriptHostAllowedOrderKeys, batchScriptHostTextOrderKeys...)
	if listOptsErr != nil {
...
func (ds *Datastore) applyHostFilters(
	...
	sqlStmt, whereParams, err = appendListOptionsWithCursorToSQLSecure(sqlStmt, whereParams, &opt.ListOptions, hostAllowedOrderKeys, hostTextOrderKeys...)
	if err != nil {
...
func (*Datastore) getBatchExecutionFilters(whereParams []interface{}, opt fleet.HostListOptions) (batchScriptExecutionJoin string, batchScriptExecutionFilter string, updatedWhereParams []interface{}) {
	...
	if opt.BatchScriptExecutionStatusFilter.IsValid() {
		...
		case fleet.BatchScriptExecutionPending:
			...
			batchScriptExecutionFilter += ` AND ((hsr.host_id IS NOT NULL AND (hsr.exit_code IS NULL AND (hsr.canceled IS NULL OR hsr.canceled = false) AND bsehr.error IS NULL)) OR (hsr.host_id is NULL AND ba.canceled = false AND bsehr.error IS NULL))`
		...
		case fleet.BatchScriptExecutionCanceled:
			...
			batchScriptExecutionFilter += ` AND ((hsr.exit_code IS NULL AND hsr.canceled = true) OR (hsr.host_id IS NULL AND bsehr.error IS NULL AND ba.canceled = true))`
		}
	}
...
func (ds *Datastore) EnrollOsquery(ctx context.Context, opts ...fleet.DatastoreEnrollOsqueryOption) (*fleet.Host, error) {
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
      ...
      LIMIT 1
		...
		_, err = tx.ExecContext(ctx, ds.dialect.InsertIgnoreInto()+` label_membership (host_id, label_id) VALUES (?, (SELECT id FROM labels WHERE name = 'All Hosts' AND label_type = 1))`+ds.dialect.OnConflictDoNothing("host_id,label_id"), hostID)
		if err != nil {
...
func (ds *Datastore) getContextTryStmt(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
	...
	if stmt := ds.loadOrPrepareStmt(ctx, query); stmt != nil {
		...
		if err == nil || !ds.dialect.IsBadConnection(err) {
			return err
		}
...
func (ds *Datastore) SetOrUpdateDeviceAuthToken(ctx context.Context, hostID uint, authToken string) error {
	...
	recentInterval := "DATE_SUB(NOW(), INTERVAL 3600 SECOND)"
	updatedAtExpr := "" // MySQL: let ON UPDATE CURRENT_TIMESTAMP handle it
	if ds.dialect.IsPostgres() {
		recentInterval = "NOW() - INTERVAL '3600 seconds'"
		updatedAtExpr = `,
			updated_at = CASE WHEN VALUES(token) = host_device_auth.token
			                 THEN host_device_auth.updated_at
	...
	stmt := `
		...
		VALUES
			(?, ?)
		` + ds.dialect.OnDuplicateKey("host_id", `previous_token = CASE
				WHEN host_device_auth.token = VALUES(token) THEN host_device_auth.previous_token
				WHEN host_device_auth.updated_at >= `+recentInterval+` THEN host_device_auth.token
				ELSE NULL END,
			token = VALUES(token)`+updatedAtExpr) + `
`
...
func (ds *Datastore) ListPoliciesForHost(ctx context.Context, host *fleet.Host) ([]*fleet.HostPolicy, error) {
	...
	LEFT JOIN (
		SELECT pl.policy_id,
			...
			SUM(CASE WHEN pl.exclude = true AND pl.require_all = true AND lm.host_id IS NOT NULL THEN 1 ELSE 0 END) AS host_exclude_all_count
...
func (ds *Datastore) replaceHostMunkiIssues(ctx context.Context, hostID uint, msgToID map[[2]string]uint) error {
	...
	if counts.CountNew < len(newIDs) {
		...
		var (
			insStmt  = `INSERT INTO host_munki_issues (host_id, munki_issue_id) VALUES %s ` + ds.dialect.OnDuplicateKey("host_id,munki_issue_id", "host_id = VALUES(host_id)")
			stmtPart = `(?, ?),`
		)
...
func (ds *Datastore) getOrInsertMunkiIssues(ctx context.Context, errors, warnings []string, batchSize int) (msgToID map[[2]string]uint, err error) {
	...
	if missing := missingIDs(); len(missing) > 0 {
		var (
			...
			insStmt   = `INSERT INTO munki_issues (name, issue_type) VALUES %s ` + ds.dialect.OnDuplicateKey("name, issue_type", "issue_type = VALUES(issue_type)")
			stmtParts = `(?, ?),`
		)
...
`
	...
	query += ` GROUP BY mdms.id, mdms.server_url, mdms.name`
...
func updateHostIssuesFailingPoliciesForSingleHost(ctx context.Context, tx sqlx.ExecerContext, dialect DialectHelper, hostID uint) error {
	// Use bare column name for critical_vulnerabilities_count — in both MySQL ODKU and PG
	// DO UPDATE SET, a bare column reference returns the existing row's value (not the
	// inserted/excluded value). VALUES(col) in MySQL ODKU returns DEFAULT when col is not in
	// the INSERT list, which would incorrectly zero out the existing count.
	stmt := `
	...
	SELECT host_id.id, COALESCE(SUM(CASE WHEN pm.passes = false THEN 1 ELSE 0 END), 0), COALESCE(SUM(CASE WHEN pm.passes = false THEN 1 ELSE 0 END), 0)
		FROM policy_membership pm
		RIGHT JOIN (SELECT CAST(? AS SIGNED) as id) as host_id
...
func updateHostIssuesFailingPolicies(ctx context.Context, tx sqlx.ExecerContext, dialect DialectHelper, hostIDs []uint) error {
	...
	// PG: !boolean is not valid; use CASE WHEN.
	// Use bare column name for critical_vulnerabilities_count — in both MySQL ODKU and PG
	// DO UPDATE SET, a bare column reference returns the existing row's value.
	var sumExpr string
	if dialect.IsPostgres() {
		sumExpr = "COALESCE(SUM(CASE WHEN pm.passes = false THEN 1 ELSE 0 END), 0)"
	} else {
		sumExpr = "COALESCE(SUM(!pm.passes), 0)"
	}
	insertStmt := fmt.Sprintf(`
	INSERT INTO host_issues (host_id, failing_policies_count, total_issues_count)
	SELECT pm.host_id, %s, %s
		FROM policy_membership pm
		WHERE pm.host_id IN (?)
		GROUP BY pm.host_id
	...
	for i := 0; i < len(criticalCounts); i += hostIssuesInsertBatchSize {
		...
		stmt := fmt.Sprintf(
			`INSERT INTO host_issues (host_id, critical_vulnerabilities_count, total_issues_count) VALUES %s
					...
					total_issues_count = host_issues.failing_policies_count + VALUES(critical_vulnerabilities_count)`),
			values,
		)
