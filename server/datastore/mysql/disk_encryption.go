...
func (ds *Datastore) ReportEscrowError(ctx context.Context, hostID uint, errorMessage string) error {
	_, err := ds.writer(ctx).ExecContext(ctx, `
INSERT INTO host_disk_encryption_keys
  (host_id, base64_encrypted, client_error) VALUES (?, '', ?) ` + ds.dialect.OnDuplicateKey("", `client_error = VALUES(client_error)`) + `
`, hostID, errorMessage)
	return err
}
...
func (ds *Datastore) QueueEscrow(ctx context.Context, hostID uint) error {
	_, err := ds.writer(ctx).ExecContext(ctx, `
INSERT INTO host_disk_encryption_keys
  (host_id, base64_encrypted, reset_requested) VALUES (?, '', TRUE) ` + ds.dialect.OnDuplicateKey("", `reset_requested = TRUE`) + `
`, hostID)
	return err
}
