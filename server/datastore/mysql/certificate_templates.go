...
func (ds *Datastore) BatchUpsertCertificateTemplates(ctx context.Context, certificateTemplates []*fleet.CertificateTemplate) ([]uint, error) {
	...
	if ds.dialect.IsPostgres() {
		...
		sqlInsertCertificate = ds.dialect.InsertIgnoreInto() + ` certificate_templates (
			name, team_id, certificate_authority_id, subject_name
...
func (ds *Datastore) CreatePendingCertificateTemplatesForNewHost(
	...
	stmt := fmt.Sprintf(`
		...
		`+ds.dialect.OnDuplicateKey("host_uuid,certificate_template_id", `
			...
			uuid = IF(host_certificate_templates.operation_type = '%s', UUID_TO_BIN(UUID(), true), host_certificate_templates.uuid),
			status = IF(host_certificate_templates.operation_type = '%s', '%s', host_certificate_templates.status),
			operation_type = IF(host_certificate_templates.operation_type = '%s', '%s', host_certificate_templates.operation_type)
