...
--
-- pg-baseline-up-to-migration: 20260724134801
--
...
CREATE TABLE public.mdm_apple_psso_keys (
    ...
    created_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT mdm_apple_psso_keys_key_type_check CHECK (((key_type)::text = ANY ((ARRAY['signing'::character varying, 'encryption'::character varying])::text[])))
);
