...
CREATE TABLE IF NOT EXISTS "host_certificate_templates" (
  ...
  "fleet_challenge" varchar(32) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS "host_device_auth" (
  ...
  "previous_token" varchar(255) DEFAULT NULL,
  "created_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  "updated_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ,
  PRIMARY KEY ("host_id"),
...
CREATE TABLE IF NOT EXISTS "host_disks" (
  ...
  "bitlocker_protection_status" smallint DEFAULT NULL,
  PRIMARY KEY ("host_id")
);
...
CREATE TABLE IF NOT EXISTS "mdm_apple_configuration_profiles" (
  "profile_id" int  NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  "team_id" int  NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS "mdm_configuration_profile_variables" (
  ...
  "apple_declaration_uuid" varchar(37) DEFAULT NULL,
...
INSERT INTO "mdm_delivery_status" ("status") VALUES ('failed'), ('verifying'), ('pending'), ('verified')
...
INSERT INTO "mdm_operation_types" ("operation_type") VALUES ('install'), ('remove')
  ON CONFLICT DO NOTHING;
...
CREATE TABLE IF NOT EXISTS "mdm_windows_enrollments" (
  ...
  "awaiting_configuration" boolean NOT NULL DEFAULT false,
  "awaiting_configuration_at" timestamp DEFAULT NULL,
  PRIMARY KEY ("id"),
...
CREATE TABLE IF NOT EXISTS "operating_systems" (
  ...
  "installation_type" varchar(20) NOT NULL DEFAULT '',
  "os_version_id" int  DEFAULT NULL,
  PRIMARY KEY ("id"),
  CONSTRAINT "idx_unique_os" UNIQUE ("name","version","arch","kernel_version","platform","display_version","installation_type")
);
...
CREATE TABLE IF NOT EXISTS "query_results" (
  ...
  PRIMARY KEY ("id"),
  CONSTRAINT "idx_query_results_query_host" UNIQUE ("query_id","host_id")
);
...
CREATE TABLE IF NOT EXISTS "software_installers" (
  ...
  "patch_query" text NOT NULL DEFAULT '',
  "http_etag" varchar(512) DEFAULT NULL,
  PRIMARY KEY ("id"),
  CONSTRAINT "idx_software_installers_team_id_title_id" UNIQUE ("global_or_team_id","title_id")
);
CREATE INDEX IF NOT EXISTS idx_software_installers_team_url ON software_installers(global_or_team_id);
...
CREATE TABLE IF NOT EXISTS "host_recovery_key_passwords" (
  ...
  "auto_rotate_at" timestamp(6) DEFAULT NULL,
  PRIMARY KEY ("host_uuid")
);
...
/* Trigger: compute declaration token from raw_json and secrets_updated_at */
CREATE OR REPLACE FUNCTION compute_declaration_token() RETURNS trigger AS $$
BEGIN
  NEW.token := decode(md5(NEW.raw_json || COALESCE(NEW.secrets_updated_at::text, '')), 'hex');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_compute_declaration_token
  BEFORE INSERT OR UPDATE ON "mdm_apple_declarations"
  FOR EACH ROW EXECUTE FUNCTION compute_declaration_token();

/* Trigger: compute enrollment_status from enrolled, installed_from_dep, is_server, is_personal_enrollment */
CREATE OR REPLACE FUNCTION compute_enrollment_status() RETURNS trigger AS $$
BEGIN
  NEW.enrollment_status := CASE
    WHEN NEW.is_server = TRUE THEN NULL
    WHEN NEW.enrolled = TRUE AND NEW.installed_from_dep = FALSE AND NEW.is_personal_enrollment = TRUE THEN 'On (personal)'
...
CREATE TRIGGER trg_compute_enrollment_status
  BEFORE INSERT OR UPDATE ON "host_mdm"
  FOR EACH ROW EXECUTE FUNCTION compute_enrollment_status();

/* Trigger: compute execution_status from exit codes, pre_install_query_output, canceled, uninstall */
CREATE OR REPLACE FUNCTION compute_execution_status() RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER trg_compute_execution_status
  BEFORE INSERT OR UPDATE ON "host_software_installs"
  FOR EACH ROW EXECUTE FUNCTION compute_execution_status();

/* Trigger: compute status (like execution_status but also checks removed flag) */
CREATE OR REPLACE FUNCTION compute_hsi_status() RETURNS trigger AS $$
BEGIN
  NEW.status := CASE
    WHEN NEW.removed = TRUE THEN NULL
    WHEN NEW.canceled = TRUE AND NEW.uninstall = FALSE THEN 'canceled_install'
    WHEN NEW.canceled = TRUE AND NEW.uninstall = TRUE THEN 'canceled_uninstall'
    WHEN NEW.post_install_script_exit_code IS NOT NULL AND NEW.post_install_script_exit_code = 0 THEN 'installed'
    WHEN NEW.post_install_script_exit_code IS NOT NULL AND NEW.post_install_script_exit_code != 0 THEN 'failed_install'
...
CREATE TRIGGER trg_compute_hsi_status
  BEFORE INSERT OR UPDATE ON "host_software_installs"
  FOR EACH ROW EXECUTE FUNCTION compute_hsi_status();

/* Seed data: fleet_variables used by tests */
INSERT INTO "fleet_variables" ("name", "is_prefix", "created_at") VALUES
  ('FLEET_VAR_NDES_SCEP_CHALLENGE', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_NDES_SCEP_PROXY_URL', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_HOST_END_USER_EMAIL_IDP', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_HOST_HARDWARE_SERIAL', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_HOST_END_USER_IDP_USERNAME', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_HOST_END_USER_IDP_USERNAME_LOCAL_PART', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_HOST_END_USER_IDP_GROUPS', FALSE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_DIGICERT_DATA_', TRUE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_DIGICERT_PASSWORD_', TRUE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_CUSTOM_SCEP_CHALLENGE_', TRUE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_CUSTOM_SCEP_PROXY_URL_', TRUE, '2025-04-22 00:00:00'),
  ('FLEET_VAR_SCEP_RENEWAL_ID', FALSE, '2025-04-30 00:00:00'),
  ('FLEET_VAR_HOST_END_USER_IDP_DEPARTMENT', FALSE, '2025-06-27 00:00:00'),
  ('FLEET_VAR_HOST_UUID', FALSE, '2025-08-08 00:00:00'),
  ('FLEET_VAR_HOST_END_USER_IDP_FULL_NAME', FALSE, '2025-08-25 00:00:00'),
  ('FLEET_VAR_SCEP_WINDOWS_CERTIFICATE_ID', FALSE, '2025-10-22 00:00:00'),
  ('FLEET_VAR_HOST_PLATFORM', FALSE, '2025-11-19 00:00:00');

/* Trigger: compute uuid text from uuid_bin bytea for calendar_events */
CREATE OR REPLACE FUNCTION compute_calendar_event_uuid() RETURNS trigger AS $$
BEGIN
  IF NEW.uuid_bin IS NOT NULL AND length(NEW.uuid_bin) = 16 THEN
    NEW.uuid := lower(
      encode(substring(NEW.uuid_bin from 1 for 4), 'hex') || '-' ||
      encode(substring(NEW.uuid_bin from 5 for 2), 'hex') || '-' ||
      encode(substring(NEW.uuid_bin from 7 for 2), 'hex') || '-' ||
      encode(substring(NEW.uuid_bin from 9 for 2), 'hex') || '-' ||
      encode(substring(NEW.uuid_bin from 11 for 6), 'hex')
    );
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_compute_calendar_event_uuid
  BEFORE INSERT OR UPDATE ON "calendar_events"
  FOR EACH ROW EXECUTE FUNCTION compute_calendar_event_uuid();

-- Tables added by migrations 20260401153000, 20260409153714, 20260409153717
-- These were missing from the original baseline generation.

CREATE TABLE IF NOT EXISTS "identity_serials" (
  "serial" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  "created_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY ("serial")
);
...
CREATE TABLE IF NOT EXISTS "acme_enrollments" (
  ...
  "host_uuid" varchar(255) NOT NULL DEFAULT '',
  "acme_directory_url" varchar(2048) NOT NULL DEFAULT '',
  "cert_chain" text,
  "created_at" timestamp DEFAULT CURRENT_TIMESTAMP,
  "updated_at" timestamp DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY ("id")
...
CREATE TABLE IF NOT EXISTS "user_api_endpoints" (
  ...
  "created_at" timestamp DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY ("id"),
  CONSTRAINT "idx_user_api_endpoints_unique" UNIQUE ("user_id", "endpoint", "method")
);
