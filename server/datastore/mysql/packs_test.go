	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				require.NoError(t, saveHostPackStatsDB(context.Background(), ds.writer(context.Background()), ds.dialect, host.TeamID, host.ID, randomPackStatsForHost(pack.ID, pack.Name, *pack.Type, schedQueries, amount))) //nolint:testifylint // require in goroutine is intentional for this stress test
			}
		}
	}()
