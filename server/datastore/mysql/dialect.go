...
type DialectHelper interface {
	...
	// FromDual returns the "FROM DUAL" table reference used by MySQL when selecting
	// literal values without a real table (e.g. INSERT INTO t SELECT ? FROM DUAL WHERE ...).
	//   MySQL:      " FROM DUAL"
	//   PostgreSQL: "" (bare SELECT without a table reference is valid)
	FromDual() string
	...
	// JsonQuote returns an expression that quotes a scalar value as a JSON string.
	//   MySQL:      JSON_QUOTE(<expr>)
	//   PostgreSQL: to_json(<expr>::text)::text
	JsonQuote(expr string) string
	...
	// JSONObjectFunc returns the SQL function name for building a JSON object.
	// The caller appends the parenthesised argument list directly to this name.
	//   MySQL:      "JSON_OBJECT"
