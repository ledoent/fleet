...
func upsertMDMAppleHostMDMInfoDB(ctx context.Context, tx sqlx.ExtContext, dialect DialectHelper, appCfg *fleet.AppConfig, fromSync, fromPersonalEnrollment bool, hostIDs ...uint) error {
	...
	_, err = tx.ExecContext(ctx, fmt.Sprintf(`
		...
		`+dialect.OnDuplicateKey("host_id", "enrolled = VALUES(enrolled), is_personal_enrollment = VALUES(is_personal_enrollment)"), strings.Join(parts, ",")), args...)
...
func (ds *Datastore) MDMAppleDDMDeclarationsToken(ctx context.Context, hostUUID string, scope fleet.PayloadScope) (*fleet.MDMAppleDDMDeclarationsToken, error) {
	...
	if ds.dialect.IsPostgres() {
		groupConcatExpr = `STRING_AGG(CONCAT(CONCAT(HEX(mad.token), COALESCE(hmad.variables_updated_at::text, '')), COALESCE(hmad.assets_updated_at::text, ''))::text, '' ORDER BY mad.uploaded_at DESC, mad.declaration_uuid ASC)`
	} else {
		groupConcatExpr = `GROUP_CONCAT(CONCAT(CONCAT(HEX(mad.token), IFNULL(hmad.variables_updated_at, '')), IFNULL(hmad.assets_updated_at, '')) ORDER BY mad.uploaded_at DESC, mad.declaration_uuid ASC separator '')`
	}
	stmt := fmt.Sprintf(`
SELECT
...
func (ds *Datastore) MDMAppleDDMDeclarationsResponse(ctx context.Context, identifier string, hostUUID string, scope fleet.PayloadScope) (*fleet.MDMAppleDeclaration, error) {
	...
	const stmt = `
SELECT
	mad.declaration_uuid, mad.raw_json, COALESCE(HEX(mad.token), '') as token, hmad.variables_updated_at, hmad.assets_updated_at
FROM
	host_mdm_apple_declarations hmad
	JOIN mdm_apple_declarations mad ON hmad.declaration_uuid = mad.declaration_uuid
WHERE
...
WHERE
	...
	AND ne.enabled = true
GROUP BY h.id, h.uuid, hmdm.installed_from_dep`, ds.dialect.JSONAgg("hmc.command_type"))
