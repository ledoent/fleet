...
SELECT
	...
	scope,
	CASE
		WHEN scope = 'User' THEN  COALESCE((SELECT nu.user_short_name FROM nano_enrollments ne INNER JOIN nano_users nu ON ne.user_id = nu.id WHERE ne.type = 'User' AND ne.enabled = true AND ne.device_id = host_uuid ORDER BY ne.created_at ASC LIMIT 1), '')
		ELSE ''
...
func (ds *Datastore) GetNanoMDMUserEnrollment(ctx context.Context, deviceId string) (*fleet.NanoEnrollment, error) {
	...
	err := sqlx.GetContext(ctx, ds.writer(ctx), &nanoEnroll, `SELECT id, device_id, type, enabled, token_update_tally
		FROM nano_enrollments WHERE type = 'User' AND enabled = true AND device_id = ? ORDER BY created_at ASC LIMIT 1`, deviceId)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
		}
...
func (ds *Datastore) bulkSetPendingMDMAppleHostProfilesDB(
	...
	WHERE
		...
		( hmap.checksum != ds.checksum ) OR COALESCE(hmap.secrets_updated_at < ds.secrets_updated_at, FALSE) OR
	...
	FROM
		${mdmAppleEntityTable} mae
			...
			JOIN ${mdmEntityLabelsTable} mel
				ON mel.${appleEntityUUIDColumn} = mae.${entityUUIDColumn} AND mel.exclude = false AND mel.require_all = true
	...
	FROM
		${mdmAppleEntityTable} mae
			...
			JOIN nano_devices nd
				ON nd.id = ne.device_id
			JOIN ${mdmEntityLabelsTable} mel
				ON mel.${appleEntityUUIDColumn} = mae.${entityUUIDColumn} AND mel.exclude = true AND mel.require_all = false
	...
	FROM
		${mdmAppleEntityTable} mae
			...
			JOIN nano_devices nd
				ON nd.id = ne.device_id
			JOIN ${mdmEntityLabelsTable} mel
				ON mel.${appleEntityUUIDColumn} = mae.${entityUUIDColumn} AND mel.exclude = false AND mel.require_all = false
			LEFT OUTER JOIN label_membership lm
				ON lm.label_id = mel.label_id AND lm.host_id = h.id
	WHERE
		(h.platform = 'darwin' OR h.platform = 'ios' OR h.platform = 'ipados') AND
...
func generateEntitiesToInstallQuery(entityType string, hostUUID string) (string, []any) {
	...
	WHERE
		...
		( hmae.${checksumColumn} != ds.${checksumColumn} ) OR COALESCE(hmae.secrets_updated_at < ds.secrets_updated_at, FALSE) OR
...
func sqlCaseMDMAppleStatus() string {
	...
	WHEN ((prof_pending != 0)
		...
		OR((fv_pending != 0)
			OR(((fv_verifying != 0)
				OR (fv_verified != 0))
			AND (hdek.base64_encrypted IS NULL OR (hdek.decryptable IS NOT NULL AND hdek.decryptable != true))))) THEN
		` + pending + `
	WHEN ((prof_verifying != 0)
		...
		OR((fv_verifying != 0)
			AND hdek.base64_encrypted IS NOT NULL AND (hdek.decryptable IS NULL OR hdek.decryptable = true))
	...
	WHEN ((prof_verified != 0)
		...
		OR (rl_verified != 0)
		OR((fv_verified != 0)
			AND hdek.base64_encrypted IS NOT NULL AND hdek.decryptable = true)) THEN
		` + verified + `
	END
`
}
...
func subqueryFileVaultVerifying() (string, []interface{}) {
	sql := `
		  ...
		  OR
		  (hmap.status = ? AND hdek.decryptable = true)
		)`
	args := []interface{}{
...
func (ds *Datastore) MDMAppleDDMDeclarationsToken(ctx context.Context, hostUUID string) (*fleet.MDMAppleDDMDeclarationsToken, error) {
	...
	if ds.dialect.IsPostgres() {
		groupConcatExpr = `STRING_AGG(CONCAT(HEX(mad.token), COALESCE(hmad.variables_updated_at::text, ''))::text, '' ORDER BY mad.uploaded_at DESC, mad.declaration_uuid ASC)`
	} else {
		groupConcatExpr = `GROUP_CONCAT(CONCAT(HEX(mad.token), IFNULL(hmad.variables_updated_at, '')) ORDER BY mad.uploaded_at DESC, mad.declaration_uuid ASC separator '')`
	}
	stmt := fmt.Sprintf(`
SELECT
...
FROM
	...
	JOIN mdm_apple_declarations mad ON hmad.declaration_uuid = mad.declaration_uuid
WHERE
	hmad.host_uuid = ? AND hmad.operation_type = ?`, groupConcatExpr)
...
func (ds *Datastore) MDMAppleDDMDeclarationsResponse(ctx context.Context, identifier string, hostUUID string) (*fleet.MDMAppleDeclaration, error) {
	...
	const stmt = `
SELECT
	mad.declaration_uuid, mad.raw_json, COALESCE(HEX(mad.token), '') as token, hmad.variables_updated_at
FROM
	host_mdm_apple_declarations hmad
	JOIN mdm_apple_declarations mad ON hmad.declaration_uuid = mad.declaration_uuid
WHERE
...
func (ds *Datastore) MDMAppleBatchSetHostDeclarationState(ctx context.Context) ([]string, error) {
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		return cleanUpOrphanedPendingRemoves(ctx, tx, ds.dialect)
	})
...
func cleanUpOrphanedPendingRemoves(ctx context.Context, tx sqlx.ExtContext, dialect DialectHelper) error {
	var found bool
	...
	deleteStmt := `DELETE r FROM host_mdm_apple_declarations r
		INNER JOIN host_mdm_apple_declarations i
			ON r.host_uuid = i.host_uuid
			AND r.token = i.token
			AND r.declaration_identifier = i.declaration_identifier
	...
	if dialect.IsPostgres() {
		deleteStmt = `DELETE FROM host_mdm_apple_declarations r
		USING host_mdm_apple_declarations i
		WHERE r.host_uuid = i.host_uuid
			AND r.token = i.token
			AND r.declaration_identifier = i.declaration_identifier
			AND r.operation_type = 'remove' AND r.status = 'pending'
			AND i.operation_type = 'install'
			AND i.status IN ('verified', 'verifying')`
	}
